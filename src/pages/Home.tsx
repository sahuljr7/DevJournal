...
export default function Home() {
  ...
  return (
    <div className={cn(
      ...
      {/* Hero Section */}
      <section className="relative h-[80vh] flex flex-col justify-center px-12 border-b border-[var(--border-color)] overflow-hidden">
        ...
        <div className="max-w-4xl">
          <motion.h1 
            ...
            className="text-[15vw] lg:text-[12vw] font-serif font-bold italic leading-[0.85] tracking-tighter mb-8"
          >
            Dev<br />Journal.
          </motion.h1>
          <motion.p 
            ...
            className="text-xl lg:text-2xl font-serif italic opacity-60 mb-12 max-w-2xl leading-relaxed"
          >
          ...
          >
            <button 
              ...
              className="px-12 py-5 bg-[var(--ink-color)] text-[var(--bg-color)] text-xs font-bold uppercase tracking-[0.2em] flex items-center gap-3 hover:scale-[1.02] transition-all shadow-xl"
            >
              Enter Dashboard
              <ArrowRight size={16} />
            </button>
            <button 
              ...
              className="px-12 py-5 border border-[var(--border-color)] text-[var(--ink-color)] text-xs font-bold uppercase tracking-[0.2em] flex items-center gap-3 hover:bg-[var(--secondary-bg)] transition-all"
            >
              <Plus size={16} />
              Initiate Record
            </button>
      ...
      <section className="grid grid-cols-1 md:grid-cols-3 border-b border-[var(--border-color)]">
        {[
          ...
          <div key={i} className="p-12 border-b md:border-b-0 md:border-r border-[var(--border-color)] last:border-0 hover:bg-[var(--secondary-bg)] transition-colors">
      ...
      <section className="px-12 py-24">
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-16 gap-8">
          <div>
            ...
            <h2 className="text-5xl font-serif font-bold italic leading-tight">Records Library.</h2>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-4">
            ...
            <div className="relative group">
              <button className="flex items-center gap-3 px-6 py-3 border border-[var(--border-color)] text-[10px] font-bold uppercase tracking-widest hover:bg-[var(--secondary-bg)] transition-all">
                Sort: {sortOption.replace('-', ' ')}
                <ChevronDown size={14} />
              </button>
              <div className="absolute right-0 top-full mt-2 w-48 bg-[var(--bg-color)] border border-[var(--border-color)] shadow-2xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-50">
                {[
        ...
        {sortedCards.length > 0 ? (
            ...
            {sortedCards.map((card) => (
              <motion.div
                ...
                className={cn(
                  ...
                  viewMode === 'grid' ? "p-8 flex flex-col h-full" : "p-6 flex items-center justify-between"
                )}
              >
                {viewMode === 'grid' ? (
                  <>
                    ...
                    <h3 className="text-2xl font-serif font-bold italic mb-4 group-hover:underline leading-tight">{card.title}</h3>
                  ...
                  <>
                    <div className="flex items-center gap-8 flex-1">
                      <span className="text-[10px] font-mono font-bold tracking-widest opacity-40 uppercase min-w-[80px]">{card.jiraId}</span>
                      <h3 className="text-xl font-serif font-bold italic group-hover:underline">{card.title}</h3>
                    </div>
                    <div className="flex items-center gap-12">
                      ...
                      <span className="text-[10px] font-bold uppercase tracking-widest opacity-40 min-w-[100px] text-right">{formatDate(card.createdAt)}</span>
                      <ArrowRight size={16} className="opacity-0 group-hover:opacity-100 transition-all text-[var(--ink-color)]" />
                    </div>
                  </>
                )}
              </motion.div>
          ...
          <div className="py-32 text-center border-2 border-dashed border-[var(--border-color)] rounded-sm">
            <h3 className="text-2xl font-serif italic opacity-40 mb-8">The library is currently vacant.</h3>
            <button 
              onClick={handleCreateNew}
              className="px-12 py-5 bg-[var(--ink-color)] text-[var(--bg-color)] text-xs font-bold uppercase tracking-[0.2em] flex items-center gap-3 mx-auto"
            >
      ...
      {/* Footer Branding */}
      <footer className="px-12 py-12 border-t border-[var(--border-color)] flex items-center justify-between opacity-30 italic font-serif text-[10px] tracking-widest uppercase mt-32">
